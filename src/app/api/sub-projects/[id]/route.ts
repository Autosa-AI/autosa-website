...
export async function PATCH(
  ...
  try {
    ...
    const target = await db.collection<SubProject>("sub_projects").findOne({ _id: new ObjectId(id) });
    if (!target) return NextResponse.json({ success: false, error: "Sub-project not found" }, { status: 404 });
    ...
    await db.collection<SubProject>("sub_projects").updateOne({ _id: new ObjectId(id) }, { $set });
    ...
    await logAction({
      ...
      documentId: id,
      details: `Updated sub-project: ${target.name}`,
    });

    return NextResponse.json({ success: true }, { status: 200 });
  } catch (err) {
    console.error("PATCH sub-project error:", err);
...
export async function DELETE(
  ...
  try {
    ...
    // Also delete all nested projects
    await db.collection("projects").deleteMany({ subProjectId: new ObjectId(id) });
    await db.collection<SubProject>("sub_projects").deleteOne({ _id: new ObjectId(id) });
    ...
    await logAction({
      ...
      collection: "sub_projects",
      documentId: id,
      details: `Deleted sub-project: ${target.name} (and its nested projects)`,
    });

    return NextResponse.json({ success: true }, { status: 200 });
  } catch (err) {
    console.error("DELETE sub-project error:", err);
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
  }
}

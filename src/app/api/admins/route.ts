...
export async function POST(req: NextRequest) {
  try {
    ...
    const db = await getDb();
    const existing = await db.collection<Admin>("admins").findOne({ email: email.toLowerCase().trim() });
    ...
    const result = await db.collection<Omit<Admin, "_id">>("admins").insertOne(newAdmin);
    ...
    return NextResponse.json({ success: true, data: { _id: result.insertedId, ...newAdmin, passwordHash: undefined } }, { status: 201 });
  } catch (err) {
